...
class A2C:
  ...
  def __init__(self,
    ...
    with tf.variable_scope("A2C"):
      ...
      self.policy_network = fully_connected_network.FullyConnectedNetwork(sess, self.observations, [None, self.action_shape, 2], "policy_network", policy_network_param)
      #
      # value network, outputs value of observation, used for advantage estimate
      self.value_network = fully_connected_network.FullyConnectedNetwork(sess, self.observations, [None], "value_network", value_network_param)
      #
      ##### Policy Action Probability #####
      #
      # isolating the mean outputted by the policy network
      self.mean_policy = tf.squeeze(self.policy_network.out[:,:,0])
      #
      # isolating the stddev outputted by the policy network, softplus is used to make sure that the stddev is positive
      self.stddev_policy = tf.nn.softplus(tf.squeeze(self.policy_network.out[:,:,1])) + 1e-5
      #
      ...
      self.action_sampled = gaussian_policy_distribution._sample_n(1)
      #
      # action sampled is cliped to environment action bounds
      self.action_sampled = tf.clip_by_value(action_sampled, self.env.action_space.low[0], self.env.action_space.high[0])
      #
      ##### Loss #####
      #
      # loss for the policy network and value network
      self.policy_network_loss = -self.gaussian_policy_distribution.log_prob(self.actions)*self.advantages
  ...
  def compute_action(self, observation):
    return sess.run(self.action_sampled, {self.observations: observation[None]})

  def compute_value(self, observations):
    return sess.run(self.value_network.out, {self.observations: observations})
  ...
  def train(self):
    ...
    for iteration in range(iterations):
      ...
      while batch_size < min_batch_size:
        ...
        while not done:
          if len(trajectories)==0 and (i%10==0):
            self.env.render()
      ...
      advantages_batch = np.concatenate(advantages)
      returns_batch = np.concatenate(returns)
      ...
      print("Average Batch Return: %.2f" % np.mean(returns_batch))
